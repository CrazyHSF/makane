...
import { ProcessDescription, CreateProcessOptions, DeepPartial } from '../common/types'
...
const generate = (descriptions: Array<ProcessDescription>): ProcessesSnapshot => ({
  version: CURRENT_SNAPSHOT_VERSION,
  cpos: descriptions.map(d => pick(d, ['name', 'options'])),
})

const serialize = (snapshot: ProcessesSnapshot): string => trimStart(`
...
const validate = (source: DeepPartial<ProcessesSnapshot>) => new Promise<ProcessesSnapshot>((resolve, reject) => {
  validator.validate(source, (errors, fields) => {
    if (errors) { reject(errors) } else {
      try {
        const snapshot = source as ProcessesSnapshot
        assert.equal(snapshot.version, CURRENT_SNAPSHOT_VERSION, 'invalid snapshot version')
